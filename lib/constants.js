...
const ALPHA1 = 28.0;
/* 省份基础 */
const STRONG_PROVINCE_BUDGET = 175000;
const NORMAL_PROVINCE_BUDGET = 100000;
const WEAK_PROVINCE_BUDGET = 40000;
const STRONG_PROVINCE_TRAINING_QUALITY = 0.8;
const NORMAL_PROVINCE_TRAINING_QUALITY = 0.6;
const WEAK_PROVINCE_TRAINING_QUALITY = 0.3;
/* 比赛日程 */
const WEEKLY_SUBSIDY_BASE = 5000;
const SUBSIDY_REP_BONUS = 220;

const COMPETITION_SCHEDULE = [
  // 调整后的原始周数（用于按 ORIGINAL_SEASON_WEEKS 缩放）
  // 第一轮：3,5,7,11,14  第二轮：17,19,21,25,28
  {week:9, name:"CSP-S1", difficulty:100, maxScore:100, numProblems:1, subtasksPerProblem:100},
  {week:17, name:"CSP-S2", difficulty:300, maxScore:400, numProblems:4},
  {week:20, name:"WC", difficulty:500, maxScore:300, numProblems:3},
  {week:30, name:"NOIP", difficulty:320, maxScore:400, numProblems:4},
  {week:40, name:"省选", difficulty:500, maxScore:600, numProblems:6, subtasksPerProblem:7},
  {week:50, name:"APIO", difficulty:500, maxScore:300, numProblems:3},
  {week:60, name:"NOI", difficulty:550, maxScore:700, numProblems:7, subtasksPerProblem:7},
  ...
  {week:74, name:"CTT-day1-2", difficulty:630, maxScore:600, numProblems:6, nationalTeam:true},
  {week:75, name:"CTT-day3-4", difficulty:660, maxScore:600, numProblems:6, nationalTeam:true},
  {week:76, name:"CTS", difficulty:720, maxScore:800, numProblems:8, nationalTeam:true},
  {week:80, name:"IOI", difficulty:740, maxScore:600, numProblems:6, nationalTeam:true, subtasksPerProblem:15}
];
...
const COMPETITION_DIFFICULTY_FACTORS = {
  ...
  "CSP-S2": [0.5, 1.2, 1.5, 2.0],
  "NOIP": [0.5, 1.5, 1.5, 2.2],
  "省选": [0.6, 1.2, 1.5, 0.8, 1.3, 1.6],
  "NOI": [0.1, 0.6, 1, 1.3, 0.8, 1.5, 1.7],
  // 国家集训队比赛难度系数
...
const WC_ENTRY_THRESHOLD = 240;   // CSP-S2 达到 240 分可参加 WC
const APIO_ENTRY_THRESHOLD = 200; // CSP-S2 达到 200 分可参加 APIO

// 明确的比赛链顺序（用于链式晋级判断）
// 国家集训队比赛不在此列表中，因为它们的晋级逻辑是特殊的
const COMPETITION_ORDER = ["CSP-S1","CSP-S2","WC","NOIP","省选","APIO","NOI"];
...
const COMPETITION_BASE_CUTOFF = {
  ...
  "NOI": {
    "强省": 0.70,      // NOI金牌线，所有省份统一为80%
    "普通省": 0.70,    // 与金牌线保持一致
    "弱省": 0.70
  },
  "WC": { "强省": 0.65, "普通省": 0.65, "弱省": 0.65 },
  "APIO": { "强省": 0.55, "普通省": 0.55, "弱省": 0.55 }
};
...
//初始能力
const KNOWLEDGE_ABLILTY_START = 35;
...
const FACILITY_UPGRADE_COSTS = {
  computer: {base:10000,grow:1.6},
  library: {base:10000,grow:1.5},
  ac: {base:3000,grow:1.4},
  dorm: {base:4000,grow:1.4},
  canteen: {base:4000,grow:1.4}
};
...
/* 训练 */
const TRAINING_BASE_KNOWLEDGE_GAIN_PER_INTENSITY = 10;
...
const TRAINING_PRESSURE_MULTIPLIER_LIGHT = 0.5;
const TRAINING_PRESSURE_MULTIPLIER_MEDIUM = 0.5;
const TRAINING_PRESSURE_MULTIPLIER_HEAVY = 1;
...
const GET_TALENT_Probability = 0.9;
...
const CONTEST_GAIN_RATIOS = {
  ...
  'WC': { knowledge: 1.0, thinking: 1.0, coding: 1.0 },
  'APIO': { knowledge: 1.0, thinking: 1.0, coding: 1.0 },
...
const OUTFIT_BASE_COST_INTERMEDIATE = 30000;
const OUTFIT_BASE_COST_ADVANCED = 60000;
const STRONG_PROVINCE_COST_MULTIPLIER = 1.5;
const WEAK_PROVINCE_COST_MULTIPLIER = 0.7;
const OUTFIT_KNOWLEDGE_BASE_BASIC = 5;
const OUTFIT_KNOWLEDGE_BASE_INTERMEDIATE = 6;
const OUTFIT_KNOWLEDGE_BASE_ADVANCED = 8;
const OUTFIT_ABILITY_BASE_BASIC = 6.0;
const OUTFIT_ABILITY_BASE_INTERMEDIATE = 8;
const OUTFIT_ABILITY_BASE_ADVANCED = 10.0;
const OUTFIT_PRESSURE_BASIC = 10;
const OUTFIT_PRESSURE_INTERMEDIATE = 15;
const OUTFIT_PRESSURE_ADVANCED = 20;
/* 声誉对外出集训/商业/媒体的影响系数 */
// 声誉越高，外出集训费用越低：最大折扣比例（相对于原价）
const OUTFIT_REPUTATION_DISCOUNT = 0.75; // 最高可减免 75%
...
const ONLINE_CONTEST_TYPES = [
  {name: "Atcoder-ABC", numProblems: 7, difficulty: 320, displayName: "Atcoder ABC"},
  {name: "Atcoder-ARC", numProblems: 5, difficulty: 530, displayName: "Atcoder ARC"},
  {name: "Atcoder-AGC", numProblems: 6, difficulty: 730, displayName: "Atcoder AGC"},
  {name: "Codeforces-Div3", numProblems: 5, difficulty: 220, displayName: "Codeforces Div.3"},
  {name: "Codeforces-Div2", numProblems: 5, difficulty: 530, displayName: "Codeforces Div.2"},
  {name: "Codeforces-Div1", numProblems: 5, difficulty: 670, displayName: "Codeforces Div.1"},
  {name: "Ucup", numProblems: 4, difficulty: 700, displayName: "Ucup"}
];
...
/* =========== 失误系统 =========== */
// 失误概率基础参数
const MISTAKE_BASE_PROBABILITY = 0.4;  // 代码能力为0时的基础失误概率
const MISTAKE_MIN_PROBABILITY = 0.2;   // 最低失误概率（代码能力>=100时）
const MISTAKE_CODING_FACTOR = 0.002;   // 代码能力对失误概率的影响系数（每点代码能力降低0.2%失误概率）

// 失误扣分参数
const MISTAKE_MIN_PENALTY = 0.50;       // 最小扣分比例（10%）
const MISTAKE_MAX_PENALTY = 0.75;       // 最大扣分比例（100%）
...
TRAINING_EFFECT_MULTIPLIER = 0.450;      // 训练效果增幅（训练后属性增加）
OUTFIT_EFFECT_MULTIPLIER = 0.800;        // 集训效果增幅（集训后属性增加）
