  ...
  const halfIndex = (game.week > halfBoundary) ? 1 : 0;
    ...
    for(let s of game.students){
      ...
      // IOI 特殊处理：检查学生是否在 ioiQualified 列表中
       if(comp.name === 'WC'){
    // 从生涯记录中寻找 CSP-S2 的得分
    const cspRecord = (game.careerCompetitions || []).find(r => r.name === 'CSP-S2');
    const studentEntry = cspRecord?.entries.find(e => e.name === s.name);
    const score = studentEntry?.score || 0;
    
    if(score >= WC_ENTRY_THRESHOLD){
      isEligible = true;
      console.log(`[资格检查] ✅ ${s.name} CSP-S2得分 ${score}，入围 WC`);
    } else {
      isEligible = false;
      console.log(`[资格检查] ❌ ${s.name} 分数不足，无法参加 WC`);
    }
  }
  // --- APIO 资格检查 ---
    ...
    if(score >= APIO_ENTRY_THRESHOLD){
      isEligible = true;
      console.log(`[资格检查] ✅ ${s.name} CSP-S2得分 ${score}，入围 APIO`);
    } else {
      isEligible = false;
      console.log(`[资格检查] ❌ ${s.name} 分数不足，无法参加 APIO`);
    }
  }
      else if(isIOI){
