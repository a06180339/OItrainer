...
(function(global){
  ...
  function distributeContestGains(totalGainCap, problems, gainType = 'knowledge'){
    ...
    const weights = problems.map(prob => {
      const scoreRatio = Math.min(Math.max(1, prob.maxScore) / Math.max(1, prob.actualScore) / 3, 1);
    ...
    const gains = weights.map(w => {
      ...
      // 知识增幅向下取整，能力增幅保留1位小数
      return (gainType === 'knowledge' ? Math.floor(rawGain) : Math.round(rawGain * Math.uniform(8, 15));
    });
