...
function getLetterGrade(val) {
  ...
  if (n > 160) {
    ...
    if (v > 160 && v < 169) {
      const offset = n - 161; // 0-based offset after 100
    ...
    if (v >= 170) {
      const tier = Math.floor((v - 170) / 160) + 1; // 110-209.999 -> tier 1, 210-309.999 -> tier 2
      const rangeStart = 170 + (tier - 1) * 160;
      const rel = (v - rangeStart) / 160.0; // [0,1)
