...
function initGame(difficulty, province_choice, student_count){
  ...
  for(let i=0;i<student_count;i++){
    ...
    newStud.knowledge_ds     = clampInt(2.5 * normal(base, 5), 40, 130);
    newStud.knowledge_graph  = clampInt(2.5 * normal(base, 5), 40, 130);
    newStud.knowledge_string = clampInt(2.5 * normal(base, 5), 40, 130);
    newStud.knowledge_math   = clampInt(2.5 * normal(base, 5), 40, 130);
    newStud.knowledge_dp     = clampInt(2.5 * normal(base, 5), 40, 130);
    try{ if(window.TalentManager && typeof window.TalentManager.assignInitialTalent === 'function') window.TalentManager.assignInitialTalent(newStud); }catch(e){}
    game.students.push(newStud);
  }
  game.updateWeather();
